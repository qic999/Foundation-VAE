...
ckpt = os.path.join(logdir, "checkpoints", "epoch=000095.ckpt")

configs_file = "configs/latent-diffusion/infer_valid_multi.yaml"
...
config = OmegaConf.load('./configs/latent-diffusion/infer_valid_multi.yaml')
data = instantiate_from_config(config.data)
data.prepare_data()
data.setup()

save_path = f'inference_valid'
...
for idx, data in tqdm(enumerate(valloader)):
    ...
    # disease_label=data['disease_label'][0]
    # breakpoint()

    data_path = os.path.join(save_path, str(f'{name}.nii.gz').replace('/', '_'))
    ...
    # data_path = os.path.join(save_path, str(f'{name}_{disease_label}.nii.gz').replace('/', '_'))
