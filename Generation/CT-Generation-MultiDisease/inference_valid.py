...
configs_file = "configs/latent-diffusion/infer_valid.yaml"
...
config = OmegaConf.load('./configs/latent-diffusion/infer_valid.yaml')
...
for idx, data in tqdm(enumerate(valloader)):
    ...
    data_path = os.path.join(save_path, str(f'{name}_disease_{disease_mask_channel[0]}_{disease_label}.nii.gz').replace('/', '_'))
