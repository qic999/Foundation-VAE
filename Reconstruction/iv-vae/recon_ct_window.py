...
def sliding_window_encode_decode(video, vae3d, window_size=64):
    ...
    # 如果视频长度大于窗口大小，使用滑窗处理
        ...
        for i in range(num_windows):
            ...
            # 编码和解码当前窗口
            with torch.no_grad():
                latent = vae3d.encode(padded_window)
                window_recon = vae3d.decode(latent)
                # 确保有batch维度
                if window_recon.dim() == 4:  # 如果缺少batch维度 (c, t, h, w)
                    window_recon = window_recon.unsqueeze(0)
...
def main_dir(input_dir, output_dir, z_dim=16, dim=96, window_size=64, skip_existing=True):
    """
    处理目录下的所有nii.gz文件
    ...
    Args:
        input_dir: 输入目录路径
        output_dir: 输出目录路径
        z_dim: IV_VAE的z维度，默认16
        dim: IV_VAE的dim维度，默认96
        window_size: 滑窗大小，默认64
        skip_existing: 是否跳过已存在的文件，默认True
    """
    # 初始化IV_VAE模型
    print(f"Loading IV_VAE model with z_dim={z_dim}, dim={dim}...")
    ...
    print(f"Model config: z_dim={z_dim}, dim={dim}\n")
...
def main_dir_batch(input_dir, output_dir, z_dim=16, dim=96, window_size=64,
    ...
    for idx, video_path in enumerate(tqdm(nii_files_to_process, desc="Processing files")):
        ...
        try:
            ...
            if t < window_size:
                ...
                with torch.no_grad():
                    latent = vae3d.encode(padded_video)
                    results = vae3d.decode(latent)
                    # 确保有batch维度
                    if results.dim() == 4:
                        results = results.unsqueeze(0)
                ...
                for batch_start in range(0, num_windows, batch_size):
                    ...
                    if windows:
                        ...
                        with torch.no_grad():
                            latents = vae3d.encode(batch_windows)
                            recons = vae3d.decode(latents)
                            # 确保有正确的形状
                            if recons.dim() == 4:
                                recons = recons.unsqueeze(0)
                            # 如果batch被合并了，需要分割
                            if recons.shape[0] != len(windows):
                                # 重新调整形状
                                recons = recons.view(len(windows), c, window_size, h, w)
...
def process_single_file_simple(video_path, save_path, z_dim=16, dim=96):
    """
...
if __name__ == '__main__':
    ...
    # IV_VAE模型参数配置
    # 可选配置: 
    # - ivvae_z4_dim64: z_dim=4, dim=64
    # - ivvae_z8_dim64: z_dim=8, dim=64
    # - ivvae_z16_dim64: z_dim=16, dim=64
    # - ivvae_z16_dim96: z_dim=16, dim=96
    z_dim = 16
    dim = 96
